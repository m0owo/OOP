...
int main() {
    ...
    map<float, vector<string>> lowest_scores;
    int rank = 1;
    for (auto const& x : scores_map) {
        lowest_scores[lowest(x.second)].push_back(x.first);
    }
    for (auto x : lowest_scores) {
        sort(x.second.begin(), x.second.end(), greater<string>());
    }
    cout << "\nMin Score Rankings: " << endl;
    for(auto it = lowest_scores.rbegin(); it != lowest_scores.rend(); ++it) {
